...
class VoteWidgetFactory extends WidgetFactory
{
    ...
    {
        $text = VoteManager::$currentVote->getType();

        switch (VoteManager::$currentVote->getType()) {
            case "RestartMap":
                $text = "Restart Map ?";
                break;
            case "NextMap":
        ...
        $label = $this->uiFactory->createLabel($text, UiLabel::TYPE_HEADER);
...
EOL
    ...
    {
        if (VoteManager::$currentVote) {
            VoteManager::$currentVote->castYes($login);
        }
    ...
    {
        if (VoteManager::$currentVote) {
            VoteManager::$currentVote->castNo($login);
        }
    }
