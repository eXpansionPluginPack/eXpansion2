...
class WindowFrameFactory extends OriginalWindowFrameFactory implements WindowFrameFactoryInterface
{
    ...
    public function build(
        ...
        $title = $this->uiFactory
            ->createLabel($name, Label::TYPE_HEADER)
            ->setPosition($sizeX/2, 7)
            ->setWidth($sizeX)
            ->setHorizontalAlign(Control::CENTER)
            ->setTextSize(4)
            ->setAreaColor("0000")
            ->setTranslate(true);
        $mainFrame->addChild($title);
        $mainFrame->addChild(
            $this->uiFactory->createLine(0, 2)->to($sizeX, 2)->setStroke(0.5)->setColor("ffff")
        );
